...
import FrmEnrollTable from '../frmTables/FrmEnrollTable'
...
const FrmEnrollment = () => {
   ...
   // FORM DATE
   const [enrolls, updateEnrolls] = useState([]);
   ...
   async function getEnrolls(){
      showLoader();
      const res = await axios.get(`${URL}enrolls/allEnrolls?page=${page}&skip=${skip}`, {timeout:5000})
         try {
            hideLoader();
            updateEnrolls(res.data.data)
   ...
   // FETCHING
   useEffect(() => {
      getEnrolls(page, skip);
   }, [page, skip])


   let {docs, totalPages, hasNextPage, hasPrevPage } = enrolls;

   // SEARCH 
   const onSearch = (search) => {
      showLoader();
      axios.get(`${URL}enrolls/allEnrolls?enNum=${search}`,
      {timeout:5000})
         .then(res => {
            hideLoader();
            updateEnrolls(res.data.data)
